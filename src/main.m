...
for i = 1:total_cuts
    ...
    if n_cuts_options_response >= n_jump_options
        ...
        render(list_frames:next_jumpable_frame, video, jumps_folder+"0_current", half_window, use_cross_fading, false);
        
        for k = 1:n_cuts_options_response
            % Render before option
            render(list_frames:rows(k), video, jumps_folder+string(k)+"_before", half_window, use_cross_fading, false);
            
            % Render option
            render(cols(k):cols(k)+min_seq-1, video, jumps_folder+string(k)+"_option", half_window, use_cross_fading, false);
        end
...
if should_finish_at_last_frame
    list_frames = [list_frames list_frames(end):n_frames];
end
